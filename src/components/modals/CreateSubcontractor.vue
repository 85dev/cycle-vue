...
import sessionStore from '@/stores/sessionStore.js'; // Import the store
...
const emit = defineEmits(['refreshSubContractors']);
...
async function submitSubContractor() {
  ...
  await apiCaller.post(`companies/${selectedCompany.value.id}/create_sub_contractor`, subcontractor, true);

  emit('refreshSubContractors');
}
...
<template>
  <v-dialog class="dialog-width">
    <template v-slot:activator="{ props: activatorProps }">
      <v-list v-if="props.origin === 'menu'" variant="tonal" density="compact" nav style="width: 100%; margin-top: -0.65em; margin-bottom: 0.8em;">
        <v-list-item v-bind="activatorProps" style="display: flex; margin-bottom: -0.6em;" nav>
          <div class="aligner">
            ...
            <v-list-item-title style="margin-right: 0.2em;">Ajouter un sous-traitant</v-list-item-title>
          </div>
        </v-list-item>
      </v-list>
        ...
        <v-card style="padding: 0.4em;">
          ...
          <v-card-actions style="margin-bottom: 0.8em;">
            ...
            <v-btn
              variant="elevated"
              color="success"
              @click="submitSubContractor(); isActive.value = false"
            >
              Ajouter
            </v-btn>
          </v-card-actions>
